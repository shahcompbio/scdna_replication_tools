...
We recommend 500kb bin size for DLP+ samples given their coverage depth of 0.01-0.1x; however, this can be adjusted depending on the coverage depth of your sample. We have successfully run PERT on samples with bin sizes as small as 20kb; however, too many bins with 0 reads can produce NaN errors during fitting if coverage is insufficient. Additionally, you must account for additional runtime when using smaller bin sizes as there will be more bins to fit.
